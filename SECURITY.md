...
- Deploy only `alo.php` and, if you use it, its digest sidecar — never the repository or tests. Do not make a public `.env` file.
- The digest may live in either of two places. `ALO_TOKEN_HASH` in the web PHP environment is the stricter option and always takes precedence; PHP-FPM `clear_env` may require an explicit pool `env[ALO_TOKEN_HASH]` entry, and LSAPI must receive its external-app environment. Alternatively `php alo.php --setup` writes `alo-hash.php` beside the probe, which needs no pool file, panel or restart.
- The sidecar is deliberately safe to keep in the document root. It stores the SHA-256 digest of a 256-bit random token, so it is not a credential: it cannot be replayed, and inverting it is infeasible. It is written `0600`, and its first statement is `exit`, so any web server willing to execute `alo.php` executes the sidecar too and returns an empty body. A regression test asserts that. Choose the environment variable anyway if your threat model includes an attacker who can read arbitrary files but not execute code.
...
- Basic credentials can remain cached until the browser closes. Use private browsing on shared machines. Rotate with `php alo.php --setup --force`, or by generating a new digest and reloading the PHP handler. Rotation invalidates the previous token immediately. There is no application logout or session cookie.
